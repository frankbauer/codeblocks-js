// dev/phaser/support.ts
var animNamesByDir = ["N", "NO", "O", "SO", "S", "SW", "W", "NW"];
var sheetIndex = [0, 1, 3, 5, 4, 6, 7, 2];
var Tiles = class {
};
...
var BaseAnimatedSprite = class {
  ...
  moveToTile(c, r) {
    var _a;
    const tile = (_a = this.game.map) == null ? void 0 : _a.getTile(c, r);
    if (tile) {
      const p = tile.center;
      this.moveTo(p.x, p.y);
    }
...
var AnimatedSprite = class _AnimatedSprite extends BaseAnimatedSprite {
  ...
  static add(game, scene, config) {
    ...
    const oy = (config.originY ? config.originY : config.tile ? (config.y - config.tile.bottom.y) / s.height + 1 : 0.5) + shiftY / s.height;
    const ox = config.originX ? config.originX : 0.5;
    s.setOrigin(ox, oy);
    const sprite = new _AnimatedSprite(game, s, config);
    return sprite;
  }
  finishedAnimation() {
...
var WalkingSprite = class _WalkingSprite extends BaseAnimatedSprite {
  constructor(game, sprite, config) {
  ...
  static add(game, scene, config) {
    ...
    s.setOrigin(ox, oy);
    const sprite = new _WalkingSprite(game, s, config);
    if (config.speed) {
  ...
  updateCurrentTile() {
    var _a;
    const tile = (_a = this.game.map) == null ? void 0 : _a.getTileAt(this.sprite.x, this.sprite.y);
  ...
  update(time, delta) {
    ...
    if (o) {
      ...
      if (o.last.p !== null) {
        ...
        this.sprite.anims.timeScale = Math.max(0.3, v * 5e-3);
...
var IsometricMap = class extends TiledMap {
  constructor(game, scene, config) {
...
var Game = class {
  ...
  pushImage(key, uri) {
    this.imagesResources.push({ key, uri });
    if (this._scene) {
  ...
  generateSpriteSheetDirections(cfg) {
    ...
    if (self._scene && this.created) {
      if (cfg.directional) {
        ...
        const conf = {
          start: cfg.frameConfig.startFrame === void 0 ? 0 : cfg.frameConfig.startFrame,
          end: cfg.frameConfig.endFrame === void 0 ? 0 : cfg.frameConfig.endFrame
        };
  ...
  addSpriteOnTile(type, c, r, texture, frame, originX, originY, speed) {
    var _a;
    const tile = (_a = this.map) == null ? void 0 : _a.getTile(c, r);
    if (tile) {
      const p = tile.center;
  ...
  addSprite(type, x, y, texture, frame, originX, originY, speed, tile) {
    const sheetCfg = this.spritesheetResources.find(
      (c) => c.key === texture
    );
    let shiftY = 0;
    if (sheetCfg) {
...
var IsometricMapGameRPC = class {
  ...
  createSpriteOnTile(msg) {
    const sprite = this.manager.createSpriteOnTile(
      ...
      msg.c,
      msg.r,
      msg.start !== void 0 ? msg.start : true,
      msg.frame !== void 0 ? msg.frame : 0
    );
    sprite.uid = msg.ID;
  ...
  setBaseSpeed(msg) {
    msg.walkingSprite.setBaseSpeed(msg.speed === void 0 ? 5 : msg.speed);
  }
};
var IsometricMapGame = class {
  ...
  init(canvasElement, outputElement, scope, runner) {
    ...
    game.onPreload = (scene, game2) => {
      this.onPreload(game2, this);
    };
    game.onCreate = (scene, game2) => {
      this.onCreate();
      scene.input.on("pointerdown", (e) => {
        var _a;
        const t = (_a = game2.map) == null ? void 0 : _a.getTileAt(e.x, e.y);
        if (t !== void 0) {
          this.onClick(t);
    ...
    game.onUpdate = (scene, game2, time, delta) => {
