...
self.importScripts('./pyodide-0.29.3/pyodide.js')
...
async function ensurePyodide() {
    ...
    // loadPyodide no longer sets a global in 0.18+; must capture the return value
    pyodide = await loadPyodide({ indexURL: './pyodide-0.29.3/' })
        ...
        case 'initialize':
            initPromise = (async () => {
                ...
                await pyodide.loadPackage(
                    [
                        'libopenblas',
                        'mpmath', 'packaging', 'pyparsing', 'python-dateutil', 'pytz', 'six',
                        'numpy', 'scipy', 'pandas', 'sympy',
                    ],
                    { messageCallback: () => {}, errorCallback: (msg) => cerr(msg) }
                )

                // second arg is now an options object {globals} in 0.18+
                pyodide.runPython(
                    `import sys
import io
...
sys.stdout = io.StringIO()`,
        ...
        case 'interpreter':
            ...
            break
        case 'start':
            // if this worker was promoted from spare before initialize finished, wait for it
            ...
            try {
                if (args) {
                    // self.pyodide never existed; use the captured pyodide instance
                    globals.set('args', pyodide.toPy(args))
                }
                ...
                // pyodide_py.eval_code_async removed in 0.18+; use runPythonAsync
                try {
                    const output = await pyodide.runPythonAsync(script, { globals })
                    if (output) {
                        clog(output)
                    }
                } catch (err) {
            ...
            if (!o.keepAlive) {
                ...
                // console module moved to pyodide.console in 0.18+
                pyodide.runPython(
                    `
    import sys
    import js
